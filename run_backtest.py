...
class SimpleBacktest:
    ...
    def _save_checkpoint(self, current_date: datetime):
        """Save checkpoint AND full results"""
        try:
            checkpoint = {
            ...
            # Also save full results (in case backtest is interrupted)
            self._save_full_results(current_date)
    ...
    async def run(self):
        ...
        try:
            while current_date <= self.end_date:
                ...
                # Progress update every 7 days (more frequent)
                if days_processed % 7 == 0:
                    progress_pct = (days_processed / total_days) * 100
                    days_remaining = total_days - days_processed
                    logger.info(f"📅 Progress: {progress_pct:.1f}% | Date: {current_date.date()} | Days left: {days_remaining}")
                    logger.info(f"   Signals found: {self.total_signals} | Trades: {len(self.trades)}")

                # Save checkpoint every 30 days
                if days_processed % 30 == 0:
                    self._save_checkpoint(current_date)
                    logger.info(f"💾 Checkpoint & results saved at {current_date.date()}\n")
    ...
    def _save_full_results(self, current_date: datetime = None):
        """Save complete results to JSON file"""
        try:
            results_file = Path("backtest_results_simple.json")

            # Calculate stats
            total_days = (current_date - self.start_date).days if current_date else (self.end_date - self.start_date).days
            avg_signals_per_day = self.total_signals / max(1, total_days)

            results = {
                'metadata': {
                    'start_date': self.start_date.isoformat(),
                    'end_date': (current_date or self.end_date).isoformat(),
                    'pairs': self.pairs,
                    'total_days': total_days,
                    'completion_status': 'completed' if current_date is None else 'in_progress'
                },
                'summary': {
                    'total_signals': self.total_signals,
                ...
                'signals_by_type': self.signals_by_type,
                'all_trades': self.trades  # Save ALL trades, not just first 100
            }

            with open(results_file, 'w') as f:
                json.dump(results, f, indent=2)

            logger.info(f"💾 Full results saved to {results_file} ({len(self.trades)} trades)")
    ...
    def _print_results(self):
        ...
        # Save complete results with ALL trades
